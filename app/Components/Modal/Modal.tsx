...
const Modal: React.FC<ModalProps> = ({ product, onClose }) => {
  ...
  const isGameItem = (item: Product | GameWishlistItem): item is GameWishlistItem =>
  ...
  return (
    ...
    <div className="fixed top-0 left-0 w-full h-full z-50 bg-black/50 flex items-center justify-center">
      <div className="bg-white p-6 rounded-lg w-[660px] relative">
        ...
        <h1 className="text-2xl font-bold mb-4 text-black">{ isGameItem(product) ?
        product.game.name : product.name }</h1>
        <div className="flex gap-2">
          <div className="flex-row">
            {isGameItem(product) ? product.game.images.slice(0, 5).map((src: string, index: number) => (
              <div
                key={index}
              ...
              >
                <Image
                  ...
                  width={400}
                  height={450}
                  alt={`${isGameItem(product) ? product.game.name : product} image ${index + 1}`}
                  className={`${
                    index === selectedImg ? "border-2 border-black" : ""
                  } h-22 w-22 rounded-lg `}
                />
  ...
  {isGameItem(product) ? (
    ...
    />
        ...
        <p className="text-black">{isGameItem(product) ? product.game.description : product.name}</p>
        <p className="mt-2 font-bold text-black">{isGameItem(product) ?product.game.Price: product.Price }</p>
        <div>
          { isGameItem(product) ? (<button
        ...
        >
          ...
          className="flex items-center justify-center gap-2 text-white bg-gray-500 rounded-lg p-2 w-[300px] mt-4 cursor-pointer 
                    transition-all duration-200 hover:scale-105 hover:bg-gray-400"
        >
          {product.isBacklog ? "Add to basket as Back-Order":"Add to Basket"}
          <FaArrowRight />
        </button>
